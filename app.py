...
def fetch_columns_mysql(table_name):
    """
    Fetch column names for the specified table from the MySQL database.
    """
    try:
        # Connect to the database
        conn = mysql.connector.connect(
        ...
        return []
def generate_sql_prompt_template(schema):
    schema_description = "\n".join(
        f"{table}({', '.join(columns)})" for table, columns in schema.items()
    )
    template = f"""
    You are an expert SQL query generator. Below is the schema for the database:

    {schema_description}

    Generate valid SQL queries for the given questions. Avoid including any extra comments or text in the output.
...
try:
    ...
    dynamic_sql_prompt_template = generate_sql_prompt_template(schema)
...
def get_dropdown_values():
    try:
        ...
        cursor.execute("SHOW COLUMNS FROM myntra_fasion;")
        db_columns = [row[0] for row in cursor.fetchall()]  # Extract column names

        # Exclude 'stock_quantity' column
        db_columns = [col for col in db_columns if col != 'Stock_Quantity']

        # If no column_names provided, use all columns except 'stock_quantity'
        if not column_names:
        ...
        column_names = [col for col in column_names if col != 'Stock_Quantity']
        ...
        for column_name in column_names:
            sql_query = f"SELECT DISTINCT {column_name} FROM myntra_fasion;"
            cursor.execute(sql_query)
...
#         cursor.execute("SHOW COLUMNS FROM nexusdb;")
...
#         sql_query = f"SELECT SUM(stock_quantity) FROM nexusdb {filter_conditions};"
...
def checkStock():
    try:
        ...
        sql_query = (
            "SELECT Product_id, BrandName, Stock_Quantity "
            "FROM myntra_fasion WHERE " + " AND ".join(where_clauses)
        )
